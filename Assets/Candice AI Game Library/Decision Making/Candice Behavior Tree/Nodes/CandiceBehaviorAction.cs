        ...
        public override CandiceBehaviorStates Evaluate()
        {
            switch (m_action(rootNode))
            {
                case CandiceBehaviorStates.SUCCESS:
                    ...
                    if(enableEvents && behaviorTreeCallback != null)
                    {
                        CandiceBehaviorTreeEventData btEvent = new CandiceBehaviorTreeEventData(m_action.Method.Name, eventType, m_nodeState);
                        rootNode.behaviorTreeCallback(btEvent);
                    }
                    return m_nodeState;
